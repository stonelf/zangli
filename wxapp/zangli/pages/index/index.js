...
Page({
  ...
  touchEnd: function (e) {
    if (!touchMove) return;
    if((new Date()-touchTime)>100 &&  Math.abs(touchDot-touchMove)>10){
      console.log(touchDot +" - "+ touchMove)
...
function getZangliData(d) {
  ...
  d=d0;
  result.currentMonth = d.getFullYear() + "年" + (d.getMonth() + 1) + "月";
  ...
  for (var i = 0; i < d0.getDay(); i++) {
    result.zangliData[0].push({ year: "　", month: "　", date: "　"},);
  }
  ...
  for (var i = d3.getDay(); i < 6; i++) {
    result.zangliData[result.zangliData.length-1].push({ year: "　", month: "　", date: "　"});
  }
